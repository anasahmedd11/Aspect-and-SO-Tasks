...
public class RoomService {
    ...
    public List<Room> getAllRooms() {
        ...
        try {
            ...
            if (cachedRoomsJson != null && !cachedRoomsJson.isEmpty()) {
                log.info("Cache hit for key: {}", ALL_ROOMS_CACHE_KEY);
                return objectMapper.readValue(cachedRoomsJson, new TypeReference<List<Room>>() {});
            }
        } catch (Exception e) {
            log.error("Cache read error for key {}: {}", ALL_ROOMS_CACHE_KEY, e.getMessage());
        }

        // 2. Fetch from database
        log.info("Cache miss for key: {}", ALL_ROOMS_CACHE_KEY);
        ...
        if (rooms != null && !rooms.isEmpty()) {
            try {
                ...
                log.info("Cached {} rooms under key: {}", rooms.size(), ALL_ROOMS_CACHE_KEY);
            } catch (Exception e) {
                log.error("Cache write error for key {}: {}", ALL_ROOMS_CACHE_KEY, e.getMessage());
            }
        }
        return rooms;
    }
